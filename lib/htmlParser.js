...
ajv.addSchema(require('../schemas/discovery-result.json'), 'discovery-result')
...
exports.parseLocationIds = function (html) {
  ...
  const locations = {}
  for (const l of jsonRaw.discover_2015.options.l) {
    locations[l.name] = l.value
  }
  return locations
}
...
exports.extractDiscoveryInfoFromDataBlob = function (html) {
  const $ = cheerio.load(html)
  const data = scrapeIt.scrapeHTML($, {
    data: {
      selector: '#pagedata',
  ...
  const albumInfos = []
  const jsonRaw = JSON5.parse(data.data)
  for (const item of jsonRaw.discover_2015.initial.server_items) {
    // console.log(item);
    const albumInfo = {
      type: item.type,
      id: item.id,
      ...
      url_hints: {
        ...
        slug: item.url_hints.slug,
        item_type: item.url_hints.item_type
      },
      featured_track: {
        file: {
          'mp3-128': item.featured_track.file.mp3
        },
      ...
      bio_image: { image_id: item.bio_image.image_id, height: item.bio_image.height, width: item.bio_image.width },
      lo_querystr: item.lo_querystr
    }
...
exports.parseDiscoveryResults = function (html, albumUrl) {
  const data = { results: this.extractDiscoveryInfoFromDataBlob(html) }
  // console.log(data);
  return data.results.reduce(function (results, result) {
    const object = assignProps(result, {}, ['url_hints', 'featured_track', 'type', 'id', 'category', 'score', 'band_id', 'item_type_id', 'publish_date', 'genre_text', 'primary_text', 'secondary_text', 'art_id', 'location_text'])
    if (ajv.validate('discovery-result', object)) {
      results.push(object)
    } else {
      console.error('Validation error on tag result: ', ajv.errorsText(), object, ajv.errors)
    }
